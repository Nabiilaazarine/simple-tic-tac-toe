...
SoundFile soundFile;
PImage player1WinImage, player2WinImage, drawImage;
boolean isMuted = false;
int muteButtonX = 470, muteButtonY = 10, muteButtonSize = 28;  // Posisi dan ukuran tombol mute
...
// Memuat file audio
  soundFile = new SoundFile(this, "musik.mp3");
  // Memutar file audio secara otomatis
  soundFile.loop();
  rewards.put(2, 10);
  rewards.put(1, -10); 
  rewards.put(0, 0); 
  
  player1WinImage = loadImage("podium.png");
  player2WinImage = loadImage("podium.png");
  drawImage = loadImage("game.png");
...
void draw(){
  ...
  // Gambar tombol mute
  fill(250);
  rect(muteButtonX, muteButtonY, muteButtonSize, muteButtonSize);
  fill(0);
  textAlign(CENTER, CENTER);
  textSize(9);
  if (isMuted) {
    text("Unmute", muteButtonX + muteButtonSize / 2, muteButtonY + muteButtonSize / 2);
  } else {
    text("Mute", muteButtonX + muteButtonSize / 2, muteButtonY + muteButtonSize / 2);
  }
    if(ai_turn){
        delay(AI_DELAY);
        AI_Turn();
...
void mousePressed() {
  // Periksa jika tombol mute diklik
...
void updateGrid(){
    ...
    if(show_scores){
      ...
      if(!player1_choice)
          ...
          fill(#FFFFFF);   
      text("P1 Menang! : "+str(p1wins), box_width/2, height*0.95);
      fill(#FFFFFF);
      text("Seri : "+str(draws), box_width+box_width/2, height*0.95);
      if(player1_choice)
          fill(#FFFFFF);   
        else
          fill(#FFFFFF);    
      if(ai_mode){
        text("AI Menang! : "+str(p2wins), 2*box_width+box_width/2, height*0.95);
        }
      else
        text("P2 Menang! : "+str(p2wins), 2*box_width+box_width/2, height*0.95);
...
boolean check_if_over(){
    ...
    int imageWidth = 125;  // Lebar gambar yang baru
    int imageHeight = 125; // Tinggi gambar yang baru
    int imageX = width/2 - imageWidth/2; // Posisi X gambar
    int imageY = height/2 - imageHeight/2; // Posisi Y gambar
    ...
    if(winner == 1){
        if(player1_choice)
          ...
          fill(#C1FFFE);
        text("Player 1 menang !", width/2, height/2 - imageHeight);
        image(player1WinImage, imageX, imageY, imageWidth, imageHeight);
         p1wins++;
    }
    else{
      if(winner == 2){
        if(player1_choice)
          ...
          fill(#FFC1C2); 
        if(ai_mode)
          text("AI menang !", width/2, height/2 - imageHeight);
        else
          text("Player 2 menang !", width/2, height/2 - imageHeight);
        image(player2WinImage, imageX, imageY, imageWidth, imageHeight);
        p2wins++;
      }
      else{
        if(winner == 0){
        fill(255);
         text("Tie !", width/2, height/2 - imageHeight);
         image(drawImage, imageX, imageY, imageWidth, imageHeight);
        draws++;
        }
        else
            print("Error");
    ...
    text("Tekan SPASI untuk main lagi", width/2, height - text_size/1);
    textSize(text_size/2.5);
    fill(#C1FFCB, 150);
